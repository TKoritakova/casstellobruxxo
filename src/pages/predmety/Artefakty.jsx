...
const Artefakty = () => (
  <div>
    ...
    <a className='game-archive-button' href="/casstellobruxxo/Artefakty1.html" target="_blank" rel="noopener noreferrer">Otevřít archiv hodiny (NonRPG 2. 11. 2025)</a>

    <h2>Výroba artefaktů</h2>
    <hr />

    <p>Výroba magických artefaktů patří mezi nejstarší a zároveň nejkomplexnější oblasti kouzelnického řemesla v Jižní Americe. Na rozdíl od běžného sesílání kouzel, která jsou pevně svázána s okamžikem a přítomností kouzelníka, představuje artefakt <span className='text-highlighter'>trvalý nosič magie</span>. Uchovává záměr svého tvůrce, paměť použitých materiálů i <span className='text-highlighter'>otisk sil</span>, které jej probudily k existenci.</p>

    <img src={uvod} className='img-in-text img-in-text-right' />
    
    <p>Artefakt nevzniká pouhým mechanickým spojením jednotlivých částí. Je výsledkem <span className='text-highlighter'>rovnováhy mezi materiálem, symbolikou, záměrem a silami stvoření</span>. Pokud některá z těchto složek chybí nebo je narušena, výsledný předmět zůstane pouze běžným objektem, případně může nést nestabilní nebo nepředvídatelnou magii. Základním krokem výroby je proto <span className='text-highlighter'>porozumění účelu artefaktu ještě před samotným započetím práce</span>. Kouzelník musí vědět, proč artefakt vytváří, komu nebo čemu má sloužit a jakým způsobem má jeho magie působit. Stejně důležité je i pochopení <span className='text-highlighter'>hranic artefaktu</span>, protože správně vytvořený předmět není pouze silný, ale také bezpečný pro svého nositele i okolí.</p>

    <p>Výroba artefaktů je v tradici Castelobruxu úzce spojena s <span className='text-highlighter'>pěti silami stvoření</span>, které lze chápat jako základní směry magie. Tyto síly nepředstavují pouze zdroj energie, ale spíše principy, skrze které magie získává svůj charakter. <span className='text-highlighter'>Sol</span>, síla slunce, je spojena s rozumem, pravdou a řádem a často se objevuje u artefaktů zaměřených na jasnost mysli, odhalování nebo orientaci. <span className='text-highlighter'>Yvy</span>, země, představuje stabilitu, růst a léčitelství a její vliv je patrný především u ochranných a podpůrných artefaktů. <span className='text-highlighter'>Yara</span>, voda, je spojena s pamětí, empatií a proměnou a její magie se často objevuje u předmětů pracujících s emocemi, myslí nebo komunikací. <span className='text-highlighter'>Ybytu</span>, vítr, symbolizuje svobodu, duchovno a spojení s neviditelnými hlasy minulosti, zatímco <span className='text-highlighter'>Tatá</span>, oheň, představuje sílu proměny, energie a samotného probuzení magie. Artefakt může být spojen s jednou silou, ale složitější předměty často pracují s <span className='text-highlighter'>kombinací více sil současně</span>.</p>

    <img src={vyroba_01} className='img-in-text img-in-text-left' />
    
    <p>Proces výroby vyžaduje od tvůrce <span className='text-highlighter'>hluboké soustředění</span> a <span className='text-highlighter'>jasnou představu výsledného účinku</span>. Kouzelník musí být schopen propojit vlastní mysl s použitými materiály a udržet mezi nimi harmonii. Pokud se tato harmonie podaří, může být magie artefaktu probuzena i bez složitých inkantací. V mnoha případech k probuzení dochází skrze některou ze sil stvoření, která slouží jako prostředník mezi materiálním světem a magií samotnou.</p>

    <p>Přestože <span className='text-highlighter'>existují tradiční postupy výroby</span>, nelze je chápat jako přesné recepty. Každý artefakt je do určité míry <span className='text-highlighter'>jedinečný</span>, protože reaguje nejen na kvalitu materiálů, ale také na osobnost tvůrce, prostředí vzniku i okamžitou rovnováhu sil stvoření. Z tohoto důvodu se v Castelobruxu klade důraz nejen na technické zvládnutí výroby, ale především na <span className='text-highlighter'>pochopení principů</span>, které za celým procesem stojí. Výroba artefaktu tak není pouze řemeslem, ale i <span className='text-highlighter'>formou dialogu mezi kouzelníkem, přírodou a magií samotnou</span>.</p>
    ...
    <p>Materiály používané při výrobě artefaktů nejsou vybírány pouze podle fyzických vlastností, ale především podle symboliky a takzvané magické paměti. Každý materiál v sobě nese <span className='text-highlighter'>určitou energii a význam</span>, které ovlivňují výsledný účinek artefaktu. Správná kombinace materiálů umožňuje propojení jejich významů a zesílení magie, zatímco nevhodná kombinace může vést k oslabení nebo nestabilitě výsledného předmětu.</p>

    <p>Níže uvedené materiály patří mezi <span className='text-highlighter'>nejčastěji používané v amazonské artefaktní tradici</span>.</p>

    <h3>Dřevo</h3>
    <p>Dřevo patří mezi nejzákladnější materiály artefaktní výroby. Je spojováno s <span className='text-highlighter'>životem, růstem, stabilitou a pamětí přírody</span>. Různé druhy dřeva však nesou odlišnou symboliku a využívají se pro rozdílné typy artefaktů.</p>

    <img src={drevo} className='img-in-text img-in-text-right' />

    <ul>
        <li><span className='text-highlighter'>Pau-brasil</span> <i>(Caesalpinia echinata)</i> – symbol života, identity a historické paměti.</li>
        <li><span className='text-highlighter'>Kapok</span> <i>(Ceiba pentandra)</i> – posvátný strom, symbol spojení světů, snů a duchů.</li>
        <li><span className='text-highlighter'>Brazilský ořech</span> <i>(Bertholletia excelsa)</i> – symbol výživy, vitality a životní síly.</li>
        <li><span className='text-highlighter'>Ipe</span> <i>(Tabebuia spp.)</i> – symbol obnovy, krásy a dlouhověkosti.</li>
        <li><span className='text-highlighter'>Jacarandá</span> <i>(Dalbergia nigra)</i> – symbol ochrany, elegance a stabilní magie.</li>
        <li><span className='text-highlighter'>Mahagon</span> <i>(Swietenia spp.)</i> – symbol síly, moci a odolnosti.</li>
        <li><span className='text-highlighter'>Cumaru</span> <i>(Dipteryx odorata)</i> – symbol štěstí, harmonie a prosperity.</li>
        ...
        <li><span className='text-highlighter'>Guaiacum</span> <i>(Lignum vitae)</i> – symbol rituální síly a vytrvalosti.</li>
        <li><span className='text-highlighter'>Palisandr amazonský</span> <i>(Dalbergia spruceana)</i> – symbol klidu mysli a mentální rovnováhy.</li>
        <li><span className='text-highlighter'>Balsa</span> <i>(Ochroma pyramidale)</i> – symbol vzduchu, snů a cest mezi světy.</li>
        <li><span className='text-highlighter'>Eukalyptus</span> <i>(Eucalyptus)</i> – symbol očisty, léčení a ochrany před nemocemi a zlými silami.</li>
        <li><span className='text-highlighter'>Guayacán</span> <i>(Guaiacum officinale)</i> – symbol dlouhověkosti a odolnosti proti temné magii.</li>
    </ul>
    

    <h3>Zpracované a univerzální materiály</h3>
    
    <p>Některé materiály nejsou spojeny s jedním konkrétním živlem, ale fungují jako <span className='text-highlighter'>nosič nebo stabilizátor</span> magie.</p>
    ...
    <ul>
        ...
        <li><span className='text-highlighter'>Keramika a hlína</span> – symbol tvoření, formování a spojení země s ohněm.</li>
    </ul>


    <h3>Kovy</h3>
    <p>Kovy jsou spojeny především se <span className='text-highlighter'>silou, vedením energie a stabilitou magie</span>.</p>
    ...
    <p>Drahokamy fungují jako <span className='text-highlighter'>zesilovače, stabilizátory nebo směrovače magie</span>.</p>
    ...
    <p>Tyto materiály nesou <span className='text-highlighter'>instinktivní nebo přirozenou magii</span>. Největší sílu mají, pokud jsou získány <span className='text-highlighter'>dobrovolně</span>.</p>

    <ul>
        <li><span className='text-highlighter'>Skleněná žába</span> <i>(Centrolenidae)</i> – vhled a citlivost na magii.</li>
        <li><span className='text-highlighter'>Šípová žába</span> <i>(Dendrobatidae)</i> – obrana a energie.</li>
        <li><span className='text-highlighter'>Paúhoř elektrický</span> <i>(Electrophorus electricus)</i> – elektřina, komunikace.</li>
        <li><span className='text-highlighter'>Chameleon</span> <i>(Chamaeleoninae)</i> – proměna, klam, adaptace, zrcadlení.</li>
        <li><span className='text-highlighter'>Papoušek</span> <i>(Psittaciformes)</i> – komunikace a spojení s přírodou.</li>
        ...
        <li><span className='text-highlighter'>Motýl</span> <i>(Lepidoptera)</i> – čistota, vzduch, krása, vnitřní světlo.</li>
        <li><span className='text-highlighter'>Mravenec</span> <i>(Formicidae)</i> – píle, kolektivní síla, společenství, vytrvalost.</li>
        <li><span className='text-highlighter'>Krovkovec stínový</span> <i>(Scutocaiman umbrosus)</i> – prastará síla a trpělivost.</li>
        <li><span className='text-highlighter'>Anakonda</span> <i>(Eunectes)</i> – životní cyklus a moc.</li>
        <li><span className='text-highlighter'>Delfín</span> <i>(Delphinidae)</i> – harmonie, přátelství, inteligence.</li>
    ...
    <p>Byliny se používají především pro <span className='text-highlighter'>mentální, duchovní nebo léčivé účinky</span>.</p>

    <img src={bylinky} className='img-in-text img-in-text-right' />
    
    <ul>
        <li><span className='text-highlighter'>Ayahuasca</span> <i>(Banisteriopsis caapi)</i> – vhled a duchovní otevření, liána duchů.</li>
        <li><span className='text-highlighter'>Chacruna</span> <i>(Psychotria viridis)</i> – vize a práce s časem.</li>
        <li><span className='text-highlighter'>Jergón sacha</span> <i>(Dracontium spruceanum)</i> – očista a obnova života.</li>
        <li><span className='text-highlighter'>Brazilský ženšen</span> <i>(Pfaffia paniculata)</i> – vitalita, imunita a životní síla.</li>
        <li><span className='text-highlighter'>Kokainovník pravý</span> <i>(Erythroxylum coca)</i> – energie, síla a vytrvalost.</li>
    ...
    <ul>
        ...
        <li><span className='text-highlighter'>Popel posvátného ohně</span> – moudrost, obnova vůle, prastaří duchové.</li>
    ...
    <p>Artefakt Cabaça da Yara je na první pohled běžně vypadající <span className='text-highlighter'>keramická nádoba</span>. Na vnější části je zdobena vlnovkami modré barvy, které připomínají rozbouřený tok řeky. Pokud je džbán naplněn vodou, z nitra artefaktu se rozezní <span className='text-highlighter'>zpěv mýtické Yary</span>. V širokém okolí dokáže <span className='text-highlighter'>zklidnit proud řeky</span>, čímž může ochránit území proti povodním, nebo částečně <span className='text-highlighter'>snížit intenzitu bouří</span>.</p>

    <p>Dnešní napodobeniny <span className='text-highlighter'>nemusí vypadat nutně stejně</span>, fungují však na podobném principu. Po naplnění nádoby vodou se v okolí rozezní <span className='text-highlighter'>tichý uklidňující ženský zpěv</span>. Dle síly artefaktu dokáže <span className='text-highlighter'>uklidnit lidskou mysl</span>, někdy však i <span className='text-highlighter'>zvířecí</span>. Druh zpěvu můžeme ovlivnit i tím, že místo vody naplníme nádobu <span className='text-highlighter'>odvarem z kouzelných bylin</span>.</p>

    <p>Pokud se na artefakt podíváme z hlediska dělení, pak bychom ho nejspíše zařadili mezi <span className='text-highlighter'>komunikační artefakty</span>, ale také mezi <span className='text-highlighter'>nástroje pro práci s přírodou</span>. Materiály využívané k vytvoření artefaktu mají kromě funkčnosti hlavně symbolický význam. Do keramiky získané z blízkosti <span className='text-highlighter'>palisandru</span> – schopnost <span className='text-highlighter'>uklidňovat mysl</span> – se přimíchává písek ze dna Amazonky. Ten funguje jako symbol samotné síly stvoření <span className='text-highlighter'>Yary</span>. Ta je důležitá pro schopnost <span className='text-highlighter'>empatie, práci s emocemi a s myslí</span>. Pírko <span className='text-highlighter'>papouška Muirá</span> dodává artefaktu schopnost vydávat zvuk. Při aktivaci se využívá i oheň, síla <span className='text-highlighter'>Tatá</span>, která je známá pro <span className='text-highlighter'>energii a aktivaci kouzel</span> celkově. Pokud všechny symboly uvedeme do <span className='text-highlighter'>harmonie</span>, spojí se všechny do jednoho a vznikne funkční artefakt.</p>

    <p><span className='text-highlighter text-underline'>Původ artefaktu</span></p>

    <p>S původní nádobou Iary se pojí legenda, že ji samotná <span className='text-highlighter'>Yara naplnila svým hlasem</span> a využívala ji k <span className='text-highlighter'>vábení mužů</span>. Tím, že dala nádobě svůj hlas, do ní přesunula i <span className='text-highlighter'>část své moci</span>, proto dokáže zklidnit vodu. Artefakt prý nalezl kmen etnické skupiny <span className='text-highlighter'>Tupinambá</span>, po kolonizaci Portugalci se dostal do <span className='text-highlighter'>Castelobruxa</span>. Artefakt si od té doby <span className='text-highlighter'>předávají ředitelé školy</span>. Kromě symboliky ho využívají i k <span className='text-highlighter'>ochraně</span> okolí školy. Od roku 1600 se začaly vyrábět i <span className='text-highlighter'>kopie tohoto artefaktu</span>, které i přes svou nesrovnatelně slabší moc bývají součástí mnoha domorodých kmenů.</p>
    ...
    <div className='green-box'>
        ...
        <p>Písek z Amazonky necháme schnout na kameni vystupujícím z vodního toku. V průběhu sedíme na okraji řeky, vnímáme její rytmus, zvuk a klid – a ten se snažíme vstřebat. Až tehdy můžeme pokračovat.</p>
        ...
        <p>Keramiku vytvarujeme do podoby, kterou přizpůsobíme nádobě. Nádoba sama si o svůj tvar řekne, proto nejde vyrobit dvě nádoby zcela stejné.</p>
        ...
        <p>Již vytvarovanou nádobu z vnější strany obložíme pírky papouška. Pírka, která papoušek tvůrci artefaktu dobrovolně dá, zvyšují moc artefaktu. Během přikládání do pírek zašeptáme tichou uklidňující melodii. Na základě melodie můžeme ovlivnit výsledný tón i efekt artefaktu.</p>

        <p><span className='text-highlighter text-underline'>5. Vypálení nádoby (Aktivace Tatá)</span></p>
        <p>Celou nádobu vložíme do ohně a necháme pírka shořet. Jejich energie se díky síle Tatá do nádoby vtiskne. Na nádobě by měla zůstat struktura pírek.</p>
    </div>
    ...
    <p>Drobný, ale dost výrazný náramek tvořený různě barevnými <span className='text-highlighter'>korály a zuby delfína</span>. Kromě schopnosti <span className='text-highlighter'>upravit svou velikost</span> ruce nositele se kolem něj při ponoření do vody vytvoří <span className='text-highlighter'>vzduchová bublina</span>. Velikost bubliny a délka efektu závisí na kvalitě materiálu a schopnostech tvůrce. Vzduchová bublina se <span className='text-highlighter'>postupně zmenšuje</span>, její magické schopnosti se pak <span className='text-highlighter'>doplní</span>, pokud se náramek vynoří nad vodu. Podle účinku by se dal zařadit mezi <span className='text-highlighter'>nástroje pro práci s přírodou</span>.</p>

    <p>Svůj význam zde samozřejmě hrají i využité materiály – <span className='text-highlighter'>korály</span>, které svým symbolem obnovy přinášejí vlastnost <span className='text-highlighter'>znovuzískání schopnosti vytvoření bubliny</span>, mimo jiné symbolizují i <span className='text-highlighter'>držení vzduchu pod vodou</span>. Tím, že pocházejí z ústí Amazonky, zajišťují fungování schopnosti <span className='text-highlighter'>jak ve sladké, tak i ve slané vodě</span>. <span className='text-highlighter'>Zuby delfína</span>, které i společně s korály symbolizují sílu <span className='text-highlighter'>Yary</span>, zároveň dodávají předmětu <span className='text-highlighter'>harmonii</span> a odkazují na <span className='text-highlighter'>původ artefaktu</span>. Pokud jsou delfínem <span className='text-highlighter'>dobrovolně darované</span>, pak je síla artefaktu o mnoho větší. <span className='text-highlighter'>Mléko kapybary</span> uvádí do <span className='text-highlighter'>rovnováhy</span> živel vody a živel vzduchu, sílu <span className='text-highlighter'>Ybytu</span>, protože artefakt potřebuje ke svému fungování síly obě. A protože není síla Ybytu reprezentována žádným materiálem, je naprosto <span className='text-highlighter'>klíčová pro aktivaci artefaktu</span>.</p>

    <p><span className='text-highlighter text-underline'>Původ artefaktu</span></p>
    <img src={pulseira_de_coral_legenda} className='img-in-text img-in-text-right' />
    
    <p>Legendy praví, že kdysi dávno, ještě před příchodem Evropanů na americký kontinent, sídlil u ústí Amazonky <span className='text-highlighter'>neznámý kmen</span> významný tím, že se uměli <span className='text-highlighter'>velmi dobře potápět</span>. Náčelník tohoto kmene prý jednou pod hladinou narazil na korál, který okolo sebe měl <span className='text-highlighter'>obrovskou vzduchovou bublinu</span>. Ten byl ale tak pevně přirostlý k zemi, že ho sám nedokázal odříznout. S tím mu prý pomohl až <span className='text-highlighter'>delfín</span>, který plul kolem. Do korálu se zakousl a podařilo se mu <span className='text-highlighter'>část odtrhnout</span>. Avšak to jenom za cenu toho, že v korálu zůstaly <span className='text-highlighter'>tři jeho zuby</span>. Náčelník poté z korálu vytvořil <span className='text-highlighter'>náramek</span> a jako <span className='text-highlighter'>symbol oběti delfína</span> do něj zakomponoval i ony tři zuby. Korál od té doby zůstává <span className='text-highlighter'>stále živý</span>. Tento artefakt dokáže vytvořit bublinu tak velkou, že se do ní vejde <span className='text-highlighter'>až skupina tří lidí</span>, zároveň je jeho účinek <span className='text-highlighter'>trvalý</span>. Podle tého legendy se později začaly vyrábět i artefakty podobné, <span className='text-highlighter'>samotné korály však schopnost odpuzovat vodu nemají</span>.</p>
    ...
    <div className='green-box'>
        ...
        <p>Všechny zuby delfína vezmeme do dlaně, stiskneme ji a v přítomnosti zvuku tekoucí vody uctíme delfínovo upřímné a dobrovolné darování.</p>
        ...
        <p>Ostatní zuby delfína a korály nasuneme na rostlinný provázek. Ten následně do vařícího mléka vložíme tak, aby nerozrušil hladinu a nevydal žádné zvuky, jinak ho směs nemusí přijmout. Během tohoto kroku musíme cítit pokoru jak vůči delfínovi, tak kapybaře.</p>

        <p><span className='text-highlighter  text-underline'>4. Aktivace artefaktu (Aktivace Ybytu)</span></p>
        <p>Náramek ze směsi vyjmeme a myslíme na sílu Ybytu. Zhluboka se nadechneme a vydechneme přímo proti náramku, čímž se na něj naladíme a sdílíme s ním svůj dech.</p>
    </div>
    ...
    <p>Malý leštěný medailon vytvořený z brazilského ametystu nebo akvamarínu. Na jeho povrch je vryt barevný <span className='text-highlighter'>symbol X s čarou vycházející ze středu nahoru</span>. Kouzelníkovi, který ovládá řeč přírody, umožní bezpečněji a silněji <span className='text-highlighter'>navázat kontakt s lesními duchy</span>. Proto se řadí mezi <span className='text-highlighter'>obřadní</span>, ale zároveň i <span className='text-highlighter'>komunikační předměty</span>. Během komunikace zároveň <span className='text-highlighter'>dodává nositeli klid do duše</span> a pomáhá <span className='text-highlighter'>bezpečnému návratu do reality</span>.</p>

    <p>Kámen Pajé je artefaktem <span className='text-highlighter'>jak síly Yvy, tak Ybytu</span>. Yvy zprostředkovává <span className='text-highlighter'>spojení s přírodou</span>, s pralesem, a mimo jiné <span className='text-highlighter'>ukotvení</span>, přičemž Ybytu, vítr, je důležitý pro schopnost samotné <span className='text-highlighter'>duchovní komunikace</span>. Svou roli zde hrají zároveň i použité materiály – výběrem drahokamu určíme, jestli bude artefakt více zaměřený na <span className='text-highlighter'>duchovní čistotu</span> (v tom případě zvolíme <span className='text-highlighter'>ametyst</span>) nebo na <span className='text-highlighter'>intuici a komunikaci</span> (a tedy využijeme <span className='text-highlighter'>akvamarín</span>). Korálky ze stromu <span className='text-highlighter'>Pau-brasil</span> symbolizují <span className='text-highlighter'>život</span>, přičemž rostliny dodávají <span className='text-highlighter'>symbol vhledu (Banisteriopsis caapi), vizí (Psychotria viridis) a životní síly (Brazilský ženšen)</span>. Barvivo z <span className='text-highlighter'>šípové žáby</span> dává artefaktu veškerou <span className='text-highlighter'>energii</span>.</p>

    <p><span className='text-highlighter text-underline'>Původ artefaktu</span></p>
    <p>Tradice vytvářet artefakty zprostředkující komunikaci s duchy pralesa je v domorodých kmenech zakotvena už od pradávna. Tyto artefakty, v Brazílii často označované jako Kámen Pajé, se postupně vyvíjely, <span className='text-highlighter'>každý kmen používal k výrobě jiné materiály</span> a také měly artefakty <span className='text-highlighter'>různý efekt</span>. V Castelobruxu se od jeho založení uchytila <span className='text-highlighter'>jedna konkrétní varianta</span>.</p>
    ...
    <div className='green-box'>
        ...
        <p>Do vybraného drahokamu pomocí diamantového hrotu vyryjeme symbol komunikace, během toho se naladíme na duchy pralesa a požádáme je o svolení, že artefakt můžeme vytvořit. Pokud cítíme zápornou odpověď, pak se do tvorby vůbec nepouštíme.</p>
        ...
        <p>Z Banisteriopsis caapi, Psychotria viridis uděláme velmi silný odvar, přimícháme barvu z šípové žáby. Vaříme tak dlouho, než ve výparech začneme vnímat vlastní hlas.</p>
        ...
        <p>Drahokam a čtyři nyní už tmavě rudé korálky navlékáme na kožený provázek, který nakonec svážeme. Složený náhrdelník položíme na zem a položíme na něj ruku. Snažíme se pocítit spojení se zemí, tím artefakt ukotvíme – zajistí bezpečný návrat z duchovní úrovně.</p>

        <p><span className='text-highlighter text-underline'>6. Aktivace (Aktivace Ybytu)</span></p>
        <p>Pálíme brazilský ženšen, pouze do úst vdechneme kouř z něj. Ten necháme v ústech tak dlouho, než se dostane do našeho vědomí, pak ho do náhrdelníku vdechneme.</p
    ></div>
    ...
    <p>Malá flétna vyřezaná z mahagonu, v jejíž dutině je uložen svazek chlupů opice Saimiri A’rua a drobné peříčko orla. Podle <span className='text-highlighter'>otevřeného či zakrytého otvoru</span> dokáže vydávat tón, který <span className='text-highlighter'>přiláká nebo naopak odradí lesní tvory</span>. Její magie působí na instinkty a magické „stopy“ zvířat spojených s pralesem. Díky tomu ji řadíme mezi <span className='text-highlighter'>nástroje pro práci s přírodou</span>.</p>

    <p>Při výrobě se zaměřujeme primárně na síly <span className='text-highlighter'>Yara a Ybytu</span>. Yara je důležitá pro <span className='text-highlighter'>empatii</span>, kterou k tvorům máme, <span className='text-highlighter'>klid a komunikaci</span>. Ybytu zachovává <span className='text-highlighter'>svobodu</span> každého živého tvora, tudíž se stará o to, aby celý artefakt <span className='text-highlighter'>nefungoval násilně</span>. Síla Ybytu je důležitá také kvůli využití <span className='text-highlighter'>mahagonového dřeva</span>, které dává flétně <span className='text-highlighter'>velkou sílu a moc</span>. <span className='text-highlighter'>Chlupy opice Saimiri A’rua</span> dodávají předmětu <span className='text-highlighter'>cit pro magii lesa</span>, <span className='text-highlighter'>peříčko orla</span> podporuje sílu Ybytu a zesiluje <span className='text-highlighter'>vhled a vedení</span>. Kapka <span className='text-highlighter'>bouřkového deště</span> symbolizuje ryzí sílu <span className='text-highlighter'>Yara</span> a využití <span className='text-highlighter'>eukalyptu a cumaru</span> přináší celý artefakt do <span className='text-highlighter'>harmonie</span>. U Mboí-Puã je výroba velmi složitá proto, že všechny materiály je náročné dostat do souznění.</p>

    <p><span className='text-highlighter text-underline'>Původ artefaktu</span></p>

    <p>První Flétna Mboí-Puã se objevila v <span className='text-highlighter'>prastarých kmenech, které sloužily duchům větru a lesa</span>. Podle legendy ji poprvé vytvořil <span className='text-highlighter'>šaman Iramã</span>, aby utišil neklidný prales a navrátil do něj rovnováhu. Každý rod měl <span className='text-highlighter'>vlastní kombinaci zvířecích částí</span> vkládaných dovnitř; některé zůstávají dodnes tajné. Verze učící se v Castelobruxu je jednou z tradičních, běžně používaných variant.</p>
    ...
    <div className='green-box'>
        ...
        <p>Mahagon se na několik hodin vloží do teplé směsi z bouřkového deště, eukalyptu a cumaru. Dřevo se tím zjemní a „otevře“ pro přijmutí záměru.</p>

        <p><span className='text-highlighter text-underline'>2. Vyřezání (Dech Ybytu)</span></p>
        <p>Facónem, tradičním jihoamerickým nožem, se opatrně vyřeže dutina a tvar flétny. Při řezbě se udržuje pravidelný rytmus dechu – symbol propojení s Ybytu, silou větru a hlasů pralesa. Zároveň se snažíme nechat se dřevem vést – vyřezávání pak půjde hladčeji.</p>
        ...
        <p>Do středu se vloží svázané opičí chlupy s jedním peříčkem orla. Tento prvek představuje „srdce hlasu“ – moudrost a instinkt v rovnováze.</p>
        ...
        <p>Tvůrce do flétny třikrát vydechne – probuzení, nasměrování, uzavření. Pokud tón při zkoušce zní čistě a dřevo lehce zavoní, je artefakt probuzen.</p>
    </div>
    ...
    <p>Artefakt Morfo’Rai je <span className='text-highlighter'>drobný, křehce působící šíp z balsového dřeva, motýlích křídel a křišťálového jádra</span>. Po vystřelení z luku nebo vyhození do vzduchu rukou <span className='text-highlighter'>plachtí vzduchem</span>. Jakmile dopadne na zem, jeho <span className='text-highlighter'>krystal se rozzáří</span> a z místa dopadu se rozvine <span className='text-highlighter'>světelná stezka vedoucí zpět k majiteli</span>. Poutník tak vždy najde cestu, i v temném lese či spletité džungli.  Při letu křídla artefaktu působí dojmem živého motýla, který ukazuje směr.</p>

    <p>Artefakt řadíme mezi <span className='text-highlighter'>posilující</span>, vzhledem k tomu, že <span className='text-highlighter'>posiluje schopnost orientace</span>, zároveň mezi <span className='text-highlighter'>nástroje pro práci s přírodou</span>, vzhledem k tomu, že dokáže <span className='text-highlighter'>číst terén a vybrat nejlepší cestu</span>. Velmi důležitá je síla stvoření <span className='text-highlighter'>Sol</span>, která symbolizuje <span className='text-highlighter'>světlo, pravdu</span> a zároveň vlastnosti <span className='text-highlighter'>odhalování a orientace</span>. <span className='text-highlighter'>Křišťál</span> reprezentuje <span className='text-highlighter'>průzračnost a světlo</span>, <span className='text-highlighter'>balsové dřevo</span> svou lehkostí připomíná <span className='text-highlighter'>proplouvání vzduchem a cestu</span>, <span className='text-highlighter'>křídla motýla</span> rodu Morpho hledají <span className='text-highlighter'>vnitřní světlo</span> a pomáhají <span className='text-highlighter'>navigovat artefakt v prostoru</span>.</p>

    <p><span className='text-highlighter text-underline'>Původ artefaktu</span></p>

    <p>Artefakt pochází ze samotného <span className='text-highlighter'>srdce amazonského pralesa</span>. Podle záznamů výzkumníků z Castelobruxa první exempláře vytvořili <span className='text-highlighter'>kouzelní šamani z kmene Moruhá</span>, který uctíval zejména noční hmyz a světélkující tvory džungle. Tito kouzelníci věřili, že motýlí křídla propojená s krystalem dokáží <span className='text-highlighter'>vést ztracené poutníky hustou a nebezpečnou džunglí</span>, kde se bez světla snadno člověk ztratí.</p>
    ...
    <div  className='green-box'>
        ...
        <p>Z balsy vyřežeme tenkou tyčku. Začneme od většího odřezku a po malých kouscích okrajujeme, než jsme s tloušťkou šípu spokojeni. Balza je měkké dřevo, je lepší pracovat po menších kouscích. Během vyřezávání myslíme na pevnou víru ve správný směr. Po vyřezání odstraníme hrubší nedostatky a nakonec povrch zahladíme, obojí pomocí navlhčeného kusu látky, v němž bude hlína smíchaná s pískem, která působí jako přírodní smirkový papír. Nakonec dřevo čistým vlhkým hadříkem očistíme.</p>

        <p><span className='text-highlighter text-underline'>2. Příprava křišťálového jádra</span></p>

        <p>Křišťál vykoupáme ve vodě a čistým hadříkem pořádně osušíme a naleštíme. Během čištění vyvoláme vzpomínku na domov.</p>
        ...
        <p>Na čistém a rovném povrchu sestavíme šíp tak, jak má vypadat - zleva vedle sebe tak umístíme křišťál, poté opracované dřevo a nakonec čtyři sušená křídla, která rozestavíme tak, jak mají být na šíp připojena. Spoje opatrně zalijeme rozehřátou pryskyřicí a necháme zaschnout.</p>

        <p><span className='text-highlighter text-underline'>4. Aktivace (Yvy)</span></p>

        <p>Tvůrce šípu vezme artefakt do ruky a symbolicky ho pustí lehce před sebe směrem k místu, které bere za svůj domov. S dotykem země se artefakt aktivuje a zapamatuje si svého tvůrce.</p>
    </div>

    <a className='game-archive-button' href="/casstellobruxxo/Artefakty2.html" target="_blank" rel="noopener noreferrer">Otevřít archiv hodiny (NonRPG 28. 11. 2025)</a>
